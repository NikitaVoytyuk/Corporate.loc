    ...
    <!-- START TRACKBACK & PINGBACK -->
        ...
        <div id="respond">
            ...
            <form action="{{ route('comment.store') }}" method="post" id="commentform">
                {{ csrf_field() }}
                @if($user = Auth::user())
                    <p class="comment-form-author"><label for="author">Name</label> <input id="name" name="name" type="text" value="{{$user->name}}" size="30" aria-required="true" /></p>
                    <p class="comment-form-email"><label for="email">Email</label> <input id="email" name="email" type="text" value="{{$user->email}}" size="30" aria-required="true" /></p>
                    <p class="comment-form-url"><label for="url">Website</label><input id="url" name="site" type="text" value="{{$user->site}}" size="30" /></p>
